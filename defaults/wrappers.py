...
class DefaultWrapper:
    ...
    def __init__(self, parameters: edict):
        ...
        for key in parameters.optimization_params.keys():
            parameters.optimization_params[key][
            ...
            if autoscale_lr:
                ...
                scaled_lr = def_lr * effective_batch_size / 256.0
    ...
    def instantiate(self):
        ...
        n_domain_classes = self.dataloaders.trainloader.dataset.num_domains
        ...
        examples_in_each_domain = (
            self.dataloaders.trainloader.dataset.examples_in_each_domain
        )
        print_ddp(
            f"The default metric has been set to : \033[94m{target_metric}\033[0m"
        )
        ...
        self.model_params.n_domain_classes = n_domain_classes
        is_multiclass = self.dataloaders.trainloader.dataset.is_multiclass
        if not is_multiclass and n_classes <= 2:
            print(
                "\033[93m Binary multi-label problem found: CHANING THE n_classes to 1\033[0m"
            )
        ...
        transfusion_dir = os.path.join(
            self.training_params.save_dir, "transfusion_inits"
        )
        self.model_params.transfusion_checkpoint = os.path.join(
            transfusion_dir, transfusion_name
        )
        ...
        if not ddp_is_on():
            if self.model.student_encoder.aux_fc is None:
                self.model.student_encoder.aux_fc = nn.Identity()
        else:
            if self.model.module.student_encoder.aux_fc is None:
                self.model.module.student_encoder.aux_fc = nn.Identity()

        print_ddp("Initialising Optimization methods . . ")
        # init and get optimizer
        optimizer_defs = self.init_optimizer(
            self.model,
        ...
        scheduler_defs = self.init_scheduler(
            self.optimizer,
            self.optimization_params.default,
        ...
        if "aux_head" in self.optimization_params:
            aux_scheduler_defs = self.init_scheduler(
                self.aux_optimizer,
                self.optimization_params.aux_head,
                len(self.dataloaders.trainloader),
                epochs,
    ...
    def init_dataloaders(self, collate_fn=None) -> edict:
        ...
        probability_bank_single_cell, probability_bank_single_cell_Loader = None, None
        probability_bank_dmso, probability_bank_dmso_Loader = None, None

        DataSet = self.dataset_mapper.get(self.dataset_params.dataset, False)
        assert (
            DataSet
        ), "Dataset not found - Plese select one of the following: {}".format(
            list(self.dataset_mapper.keys())
        )
        ...
        if self.training_params.knn_eval_every or not self.is_supervised:
            ...
            feature_bank_set = DataSet(
                self.dataset_params, mode="train", fb=True, use_subset=knn_basesize
            )
            # Use validation transform when setting up prototype vectors
            feature_bank_set.transform = valset.transform
            feature_bank_set.resizing = valset.resizing

        if self.training_params.probability_sampling:
            probability_bank_single_cell = DataSet(
                self.dataset_params, mode="train", fb=True, fb_center=True
            )
            # Use validation transform when setting up prototype vectors
            probability_bank_single_cell.transform = valset.transform
            probability_bank_single_cell.resizing = valset.resizing

            probability_bank_dmso = DataSet(
                self.dataset_params, mode="train", fb=True, fb_center=True, df_dmso=True, use_subset=knn_basesize
            )
            # Use validation transform when setting up prototype vectors
            probability_bank_dmso.transform = valset.transform
            probability_bank_dmso.resizing = valset.resizing
        ...
        print("if ddp is on: ", ddp_is_on())
        # distributed sampler
        if ddp_is_on():
            train_sampler = DS(
                trainset, num_replicas=self.visible_world, rank=self.device_id
            )
            if feature_bank_set is not None:
                feature_bank_sampler = DS(
                    feature_bank_set,
                    num_replicas=self.visible_world,
                    shuffle=False,
                    rank=self.device_id,
                )
        ...
        # print(trainset.__getitem__(0).shape)

        trainLoader = DataLoader(
            trainset, **self.dataloader_params["trainloader"], sampler=train_sampler
        )
        ...
        if feature_bank_set is not None:
            data_params_copy_feature_bank = deepcopy(
                self.dataloader_params["valloader"]
            )
            data_params_copy_feature_bank["shuffle"] = False
            feature_bank_Loader = DataLoader(
                feature_bank_set,
                **data_params_copy_feature_bank,
                sampler=feature_bank_sampler,
            )

        if probability_bank_single_cell is not None:
#             data_params_copy_feature_bank["batch_size"] = 20
            data_params_copy_feature_bank["batch_size"] = 1
            data_params_copy_feature_bank["drop_last"] = False
            probability_bank_single_cell_Loader = DataLoader(
                probability_bank_single_cell,
                **data_params_copy_feature_bank,
                sampler=feature_bank_sampler,
            )
            probability_bank_dmso_Loader = DataLoader(
                probability_bank_dmso,
                **data_params_copy_feature_bank,
                sampler=feature_bank_sampler,
            )
        ...
        return edict(
            {
                ...
                "probability_loader": probability_bank_single_cell_Loader,
                "dmso_loader": probability_bank_dmso_Loader
            }
        )
    ...
    def init_model(self) -> Classifier:
        ...
        if self.transfer_learning_params.use_pretrained:
            ...
            print_ddp(
                "\033[1mLoading pretrained model : {}\033[0m".format(
    ...
    def init_optimizer(model, optimization_params: edict, aux_params=None) -> edict:
        ...
        if aux_params is not None:
            aux_optimizer = aux_opt(
                DefaultWrapper.get_params_groups(model, included_aux), **aux_opt_params
            )
        else:
            aux_optimizer_type = optimizer_type
            aux_optimizer = opt(
                DefaultWrapper.get_params_groups(model, included_aux), **opt_params
            )
        # handling LARS
        ...
        if lars_params.use and effective_batch_size >= lars_params.batch_act_thresh:
            ...
            optimizer = LARS(
                optimizer=optimizer,
                eps=lars_params.eps,
                trust_coef=lars_params.trust_coef,
            )
        ...
        return edict(
            {
                ...
                "aux_optimizer_type": aux_optimizer_type,
            }
        )
    ...
    def init_scheduler(
        ...
        for scheduler_type in scheduler_types:
            ...
            if sch.__name__ == "OneCycleLR":
                ...
                sch_params.update(
                    {"max_lr": max_lr, "steps_per_epoch": steps_per_epoch}
                )
    ...
    def init_criteria(self):
        ...
        if self.task == "classification":
            ...
            raise NotImplementedError(
                "Only classification tasks are implemented for now"
            )
    ...
    def update_augmentation_strategy(self, parameters):
        ...
        print("aug path: ", parameters["dataset_params"]["augmentation_pipeline"])
        ...
        print("augs: ", os.path.isfile(new_strategy_dir), new_strategy_dir)
    ...
    @property
    def parameters(self):
        return edict({key: getattr(self, key) for key in self.param_attributes})
