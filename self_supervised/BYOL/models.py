...
class BYOL_loss(nn.Module):
    ...
    def forward(self, x, y):
        assert (
            len(x) == len(y) <= 2
        ), f"Expecting at most 2 augmented views but {len(x)} found"
        loss = []
...
class BARLOW_loss(nn.Module):
    def __init__(
        self, barlow_loss_weight=0.75, out_dim=2048, barlow_lambda_off_diag=0.95
    ):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        # Normalize by batch size, multiply by two as samples are used twice by
        # deatatching on sample each time
        c.div_(batch_size * 2.0)
        # c.div_(batch_size)
        if ddp_is_on():
            dist.all_reduce(c)
            synchronize()
...
class BYOLHead(BaseModel):
    def __init__(self, in_size, out_size, hidden_size=4096, num_layers=2):
        ...
        layerlist.append(
            (
                "fc_out",
                nn.Sequential(
                    nn.Linear(hidden_size, out_size), nn.BatchNorm1d(out_size)
                ),
            )
        )
    ...
    def forward(self, x):
        assert isinstance(
            x, torch.Tensor
        ), f"Expecting single view tensor but found {type(x)}"
        return self.net(x)
...
class BYOL(BaseModel):
    def __init__(
        ...
        if use_momentum:
            # Note that I use BN for the projection MLP since I got better
            # resutls during the early steps
            ...
            projection_hidden_size = (
                4096 if projection_hidden_size is None else projection_hidden_size
            )
            prediction_hidden_size = (
                4096 if prediction_hidden_size is None else prediction_hidden_size
            )
            if use_barlow:
                self.loss_fn = BARLOW_loss(
                    barlow_loss_weight=0.75, out_dim=prediction_hidden_size
                )
        else:
            ...
            projection_hidden_size = (
                2048 if projection_hidden_size is None else projection_hidden_size
            )
            prediction_hidden_size = (
                512 if prediction_hidden_size is None else prediction_hidden_size
            )
        ...
        self.online_encoder.fc = BYOLHead(
            input_dim,
            projection_size,
            projection_hidden_size,
        ...
        self.predictor = Prediction_MLP(
            projection_size, projection_size, prediction_hidden_size
        )

        # modify target model
        self.target_encoder = target_encoder
        self.target_encoder.fc = BYOLHead(
            input_dim,
            projection_size,
            projection_hidden_size,
            num_layers=bottleneck_layers,
        )
        self.target_encoder.fc.load_state_dict(
            deepcopy(self.online_encoder.fc.state_dict())
        )
        ...
        self.momentum_scheduler = CosineSchedulerWithWarmup(
            base_value=moving_average_decay, final_value=1.0, iters=momentum_iters
        )
    ...
    def ema_update(self, it):
        ...
        for online_params, target_params in zip(
            ...
            target_params.data = self.ema_updater(
                online_params.data, target_params.data
            )
    ...
    def forward(self, x, return_embedding=False, adverserial_targets=None):
        with autocast(self.use_mixed_precision):
            ...
            x_online, x_emb, aux_out = zip(
                *[self.online_encoder(x_view, return_embedding=True) for x_view in x]
            )
            ...
            with torch.no_grad():
                ...
                # zero indexing for the selection of the output head and not
                # the aux head
                x_target = [target_encoder(x_view)[0] for x_view in x][::-1]
