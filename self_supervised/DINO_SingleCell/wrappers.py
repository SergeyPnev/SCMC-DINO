...
from .models import DINOCropsProbabilitySampling
# from .invaemodule import FinVAEmodule
from self_supervised.BYOL.wrappers import BYOLWrapper

class ProbabilitySamplingWrapper(BYOLWrapper):
    def __init__(self, parameters, median=False):
    ...
    def init_model(self):
        ...
        if hasattr(transformers, self.model_params.backbone_type):
            ...
            student, teacher = Classifier(self.model_params), Classifier(
                self.model_params
            )
        ...
        if self.transfer_learning_params.use_pretrained:
            ...
            load_from_pretrained(student, pretrained_path, strict=False, drop_fc=True)
        ...
        model = DINOCropsProbabilitySampling(student, teacher, momentum_iters)
