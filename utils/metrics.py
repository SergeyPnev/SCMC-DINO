...
#         print("top5: ", torch.topk(y_pred, 5, dim=1))
#         y_pred_top_5 = torch.topk(y_pred, 5, dim=1)
        y_pred = y_pred.max(dim=1)[1].data

        y_true = y_true.flatten().detach().cpu().numpy()
        y_pred = y_pred.flatten().detach().cpu().numpy()
#         y_pred_top_5 = y_pred_top_5.flatten().detach().cpu().numpy()
        self.truths += y_true.tolist()
        self.predictions += y_pred.tolist()
        self.roc_preds += roc_pred.tolist()
#         self.predictions_top_5 += y_pred_top_5.tolist()
...
#             predictions_top_5 = np.asarray(sum(dist_gather(self.predictions_top_5), []))
        else:
            truths = self.truths
            predictions = self.predictions
            roc_preds = self.roc_preds
#             predictions_top_5 = self.predictions_top_5
...
#         print("truths: ", truths)
#         print("predictions_top_5: ", predictions_top_5)
#         accuracy_top_5 = metrics.top_k_accuracy_score(truths, predictions_top_5, k=5)
...
#                 self.prefix + "accuracy_top_5:": round(accuracy_top_5, 3),
            }
        )
...
class MultiLabelClassificationMetrics:
    ...
    def get_value(self, use_dist=True):
        ...
        accuracy = metrics.accuracy_score(truths, predictions)
        precision = metrics.precision_score(
            truths, predictions, average="macro", labels=self.labels, zero_division=0
        )
        recall = metrics.recall_score(
            truths, predictions, average="macro", labels=self.labels, zero_division=0
        )
