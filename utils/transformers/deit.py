...
class ChannelAgnosticPatchEmbed(nn.Module):  # type: ignore[misc]
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        num_patches = (img_size // patch_size) * (img_size // patch_size)
        self.img_size = img_size
        ...
        self.num_patches = num_patches
        self.embed_dim = embed_dim
        # channel-agnostic has a single projection for all chans
...
class VisionTransformer(nn.Module):
    """ Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=256, embed_dim=768, depth=12,
    ...
    def forward(self, x, return_before_head=False, patch_drop=0., return_attention=False):
        ...
        for end_idx in idx_crops:
            if return_attention:
                _h, attn = self.forward_features(torch.cat(x[start_idx:end_idx]), patch_drop, return_attention)
            else:
                _h = self.forward_features(torch.cat(x[start_idx:end_idx]), patch_drop, return_attention)
            _z = self.fc(_h)
            if start_idx == 0:
                h, z = _h, _z
            else:
    ...
    def forward_features(self, x, patch_drop, return_attention=False):
        B = x.shape[0]
        ...
        # if we mask based on attention
        if isinstance(patch_drop, torch.Tensor):
            B, _, D = x.shape  # batch, length, dim
            cls_save = x[:, 0, :].unsqueeze(dim=1)
            x = x[:, 1:, :]
            x = torch.gather(x, dim=1, index=patch_drop.unsqueeze(-1).repeat(1, 1, D))
...
def deit_tiny(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights(
            "deit_tiny", patch_size, kwargs["pretrained_type"]
        )
...
def deit_small(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights(
            "deit_small", patch_size, kwargs["pretrained_type"]
        )
        model.load_state_dict(checkpoint)
    return model
...
def deit_base(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights(
            "deit_base", patch_size, kwargs["pretrained_type"]
        )
        model.load_state_dict(checkpoint)
    return model
...
def deit_huge(patch_size=14, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights(
            "deit_huge", patch_size, kwargs["pretrained_type"]
        )
        model.load_state_dict(checkpoint)
    return model
